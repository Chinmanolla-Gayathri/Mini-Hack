...
router.post('/getAllTQScores', async (req, res) => {
  ...
  for (const test of tests) {
    ...
    try {
      ...
      let mapping = doc.mappings.find(m => Number(m.raw_score) === Number(raw_score));

      if (!mapping) {
        const maxRaw = Math.max(...doc.mappings.map(m => Number(m.raw_score)));
        if (Number(raw_score) > maxRaw) {
          mapping = doc.mappings.find(m => Number(m.raw_score) === maxRaw);
        }
      }
